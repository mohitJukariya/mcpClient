...
export class ChatService implements OnModuleInit {
    ...
    async processMessage(message: string, sessionId?: string, userId?: string): Promise<ChatResponseDto> {
        try {
            ...
            // Clean up any remaining disclaimers from final response
            finalResponse = this.removeDisclaimers(finalResponse);
    ...
    } private removeDisclaimers(response: string): string {
        let cleaned = response;

        // Remove various disclaimer patterns (case insensitive)
        cleaned = cleaned.replace(/\s*\(Note:.*?\)/gi, '');
        cleaned = cleaned.replace(/\s*Note:.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*\(This response is generated by.*?\)/gi, '');
        cleaned = cleaned.replace(/\s*This response is generated by.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*Always check.*?before making transactions.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*This is a simulated conversation.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*The tool used.*?is not actually implemented.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*does not have real-time data.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*and does not have real-time data.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*\(.*?AI.*?real-time.*?\)/gi, '');
        cleaned = cleaned.replace(/\s*Please verify.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*Remember to.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*\(Disclaimer:.*?\)/gi, '');
        cleaned = cleaned.replace(/\s*Disclaimer:.*?(?=\n|$)/gmi, '');
        cleaned = cleaned.replace(/\s*\(Please note.*?\)/gi, '');
        cleaned = cleaned.replace(/\s*Please note.*?(?=\n|$)/gmi, '');
